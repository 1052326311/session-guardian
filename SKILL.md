...
Session Guardian v2.0 基于实际使用中发现的痛点，新增三大核心能力：
...
整合VA7的3个核心机制，升级到v2.0，确保无bug后发布到ClawHub
...
```json
{
  ...
  "sessions": [
    {
      "agent": "main",
      "last_message": "开始升级 session-guardian 到 v2.0",
...
│   │   └── 2026-03-02.tar.gz
│   └── ...
├── large-sessions/           # 过大的 session 文件备份（v2.0新增）
│   └── 20260303/
│       └── dev-lead_302f8efe.jsonl
├── session-isolation-report-*.md  # Session 隔离报告（v2.0新增）
...
temp/                         # 计划文件目录（v2.0新增）
├── 智能巡检产品演示材料-plan.md
├── session-guardian-v2-upgrade-plan.md
└── recovery-*.json           # 恢复文件

Assets/Projects/              # 已完成任务归档（v2.0新增）
...
v2.0 升级是在实际使用中发现的新需求：复杂任务跨越多个 session 时状态难以追踪，跨渠道使用时容易混淆，Gateway 重启后任务丢失。因此增强了项目管理和 Session 隔离能力。
